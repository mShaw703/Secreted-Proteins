...
library(rowr)
```
## Functions Needed for multiple sources 
```{r}
...
uniprot.resultnscore <- function(uniprot.results.file){
  ...
  uniprotresults$Protein_Name <- gsub('\\s+', '', uniprotresults$Protein_Name)
  uniprotresults %>% group_by(Protein_Name) %>% distinct() -> uniprotresults
  uniprotresults %>% filter(!grepl("hCG_", Protein_Name)) -> uniprotresults
...
organelleDB.trans$Protein_Name <- gsub('\\s+', '', organelleDB.trans$Protein_Name)
organelleDB.trans %>% group_by(Protein_Name) %>% distinct() -> organelleDB.trans
as.data.frame(organelleDB.trans) -> organelleDB.trans
...
as.data.frame(compartments_trans) -> compartments_trans
compartments_trans$Protein_Name <- gsub('\\s+', '', compartments_trans$Protein_Name)
compartments_trans %>% group_by(Protein_Name) %>% distinct() -> comparmtments_trans
colnames(compartments_trans)[2] <- "compartments.Score"

zero.onescore(compartments_secr) -> compartments_secr
as.data.frame(compartments_secr) -> compartments_secr
compartments_secr$Protein_Name <- gsub('\\s+', '', compartments_secr$Protein_Name)
compartments_secr %>% group_by(Protein_Name) %>% distinct() -> compartments_secr
...
proteinatlas.secr <- as.data.frame(proteinatlas.secr)
proteinatlas.secr$Protein_Name <- gsub('\\s+', '', proteinatlas.secr$Protein_Name)
proteinatlas.secr %>% group_by(Protein_Name) %>% distinct() -> proteinatlas.secr
colnames(proteinatlas.secr)[2]<- "proteinatlas.Score"
as.data.frame(proteinatlas.secr) -> proteinatlas.secr

zero.onescore(proteinatlas.trans) -> proteinatlas.trans
proteinatlas.trans <- as.data.frame(proteinatlas.trans)
proteinatlas.trans$Protein_Name <- gsub('\\s+', '', proteinatlas.trans$Protein_Name)
proteinatlas.trans %>% group_by(Protein_Name) %>% distinct() -> proteinatlas.trans
colnames(proteinatlas.trans)[2]<- "proteinatlas.Score"
as.data.frame(proteinatlas.secr) -> proteinatlas.trans
```
...
# Call zeroone function from before
zero.onescore(CSPA.list) -> CSPA.trans
as.data.frame(CSPA.trans) -> CSPA.trans
CSPA.trans$Protein_Name <- gsub('\\s+', '', CSPA.trans$Protein_Name)
CSPA.trans %>% group_by(Protein_Name) %>% distinct() -> CSPA.trans
...
uniprot.resultnscore("other.LOCATE.secr.results.tab") -> LOCATE.secr.other
as.data.frame(LOCATE.secr.other) -> LOCATE.secr.other

uniprot.resultnscore("other.LOCATE.trans.results.tab")-> LOCATE.trans.other
as.data.frame(LOCATE.trans.other) -> LOCATE.trans.other
...
as.data.frame(LOCATE.secr) -> LOCATE.secr
...
as.data.frame(LOCATE.trans) -> LOCATE.trans
...
GO.trans$Protein_Name <- gsub('\\s+', '', GO.trans$Protein_Name)
...
GO.secr$Protein_Name <- gsub('\\s+', '', GO.secr$Protein_Name)
...
UniprotKB.secr$Protein_Name <- gsub('\\s+', '', UniprotKB.secr$Protein_Name)
...
UniprotKB.trans$Protein_Name <- gsub('\\s+', '', UniprotKB.trans$Protein_Name)
...
Goldstd.secr %>% mutate(Standard_Protein = "Confirmed") -> Goldstd.secr
colnames(Goldstd.secr)[2] <- "Goldstd.Score"
...
Goldstd.trans %>% mutate(Standard_Protein = "Confirmed") -> Goldstd.trans
colnames(Goldstd.trans)[2] <- "Goldstd.Score"
...
final.lists(list(organelleDB.trans, compartments_trans, proteinatlas.trans, CSPA.trans, LifeDB.trans, LOCATE.trans, LocDB.trans, GO.trans, UniprotKB.trans, Goldstd.trans)) -> trans.list.total
as.data.frame(trans.list.total) -> trans.list.scores
trans.list.scores$Standard_Protein[trans.list.scores$Standard_Protein == 0] <- "Uncetertain"




final.lists(list(compartments_secr, proteinatlas.secr, LOCATE.secr, GO.secr, UniprotKB.secr, Goldstd.secr)) -> secr.list.total
as.data.frame(secr.list.total) -> secr.list.scores
secr.list.scores$Standard_Protein[secr.list.scores$Standard_Protein == 0] <- "Uncertain"


# Remove any protein names that dont apply (cleaning up list)
cleanterms <- function(mydata){
    mydata %>% filter(!grepl("hCG_", Protein_Name)) -> mydata
    mydata %>% filter(!grepl("human", Protein_Name)) -> mydata
    mydata %>% filter(!grepl("tcag", Protein_Name)) -> mydata
...
# Write Final Tables
# Transmembrane
cleanterms(trans.list.scores) -> trans.list.scores
as.data.frame(trans.list.scores) -> trans.list.scores
write.table(trans.list.scores, "trans.list.scores.tsv", quote = FALSE, sep = '\t')

# Secreted
cleanterms(secr.list.scores) -> secr.list.scores
as.data.frame(secr.list.scores) -> secr.list.scores
write.table(secr.list.scores, "secr.list.scores.tsv", quote = FALSE, sep = '\t')
